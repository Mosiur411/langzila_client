...
import findVoice from '../../../../assets/QuizImg/findVoice4.png'
...
// Audio
import Audio1 from '../../../../assets/QuizImg/HindiAudio/प21.mp3'
import Audio2 from '../../../../assets/QuizImg/HindiAudio/फ22.mp3'
import Audio3 from '../../../../assets/QuizImg/HindiAudio/ब23.mp3'
import Audio4 from '../../../../assets/QuizImg/HindiAudio/भ24.mp3'
import Audio5 from '../../../../assets/QuizImg/HindiAudio/म.25.mp3'
import Audio6 from '../../../../assets/QuizImg/HindiAudio/स32.mp3'
import Audio7 from '../../../../assets/QuizImg/HindiAudio/र27.mp3'
import Audio8 from '../../../../assets/QuizImg/HindiAudio/ल28.mp3'
import Audio9 from '../../../../assets/QuizImg/HindiAudio/श30.mp3'
import Audio10 from '../../../../assets/QuizImg/HindiAudio/ष31.mp3'
...
const HindiEasyQ4 = () => {
    ...
    // Get Quiz Ans Data
    useEffect(() => {
        fetch(`https://langzila.herokuapp.com/hindiEasyQAns4/${email}`, {
            method: 'GET'
        })
            .then(res => res.json())
            .then(data => {
    ...
    const selectAnswer = async (quizAns, quz, Allans, isData) => {
        ...
        if (!isData) {
            ...
            if (QuizData) {
                try {
                    const { data } = await axios.post(`https://langzila.herokuapp.com/hindiEasyQ4`, QuizData, {
                        method: 'POST'
                    });
                    ...
                    showScore ?
                        ...
                        <div className='h-screen  w-full bg-white flex flex-col items-center justify-center'>
                            ...
                            <div className='xl:w-1/2 w-2/3'>
                                <div className=' text-left text-purple-900 text-xl font-bold'>{count + 1} <span className=' text-black'>/</span> {quizs.length}</div>
                                ...
                                <div className=' grid grid-cols-2 gap-6 mt-10'>

                                    {
                                        quizs[count]?.quiz_answers?.map((quizAns, idx) => <button onClick={() => { selectAnswer(quizAns, { quz: quizs[count]?.Latter }, { AllAns: (quizs[count]?.quiz_answers) }, isData) }} key={idx}
                                            className={` p-4 bg-purple-900 text-white hover:bg-black hover:text-white rounded shadow-md text-lg font-semibold transition duration-200`}>{quizAns?.answere}</button>)
                                    }
                                </div>
                            </div>
                            <div className=' mt-5'>
                                <progress className="progress progress-black  xl:w-[870px] w-[550px] " value={progressValue} max="100"></progress>
                            </div>
                        </div>
            }
        </div >
