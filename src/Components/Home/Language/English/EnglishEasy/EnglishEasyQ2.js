...
const EnglishEasyQ2 = () => {
    ...
    // Get Quiz Ans Data
    useEffect(() => {
        fetch(`https://langzila.herokuapp.com/englishEasyQAns2/${email}`, {
            method: 'GET'
        })
            .then(res => res.json())
            .then(data => {
    ...
    const selectAnswer = async (quizAns, quz, Allans, isData) => {
        ...
        if (!isData) {
            ...
            if (QuizData) {
                try {
                    const { data } = await axios.post(`https://langzila.herokuapp.com/englishEasyQ2`, QuizData, {
                        method: 'POST'
                    });
