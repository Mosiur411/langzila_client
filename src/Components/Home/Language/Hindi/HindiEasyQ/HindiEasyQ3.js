...
import findVoice from '../../../../assets/QuizImg/findVoice3.png'
...
// Audio
import Audio1 from '../../../../assets/QuizImg/HindiAudio/ट11.mp3'
import Audio2 from '../../../../assets/QuizImg/HindiAudio/ठ12.mp3'
import Audio3 from '../../../../assets/QuizImg/HindiAudio/ड13.mp3'
import Audio4 from '../../../../assets/QuizImg/HindiAudio/ढ14.mp3'
import Audio5 from '../../../../assets/QuizImg/HindiAudio/ण15.mp3'
import Audio6 from '../../../../assets/QuizImg/HindiAudio/त16.mp3'
import Audio7 from '../../../../assets/QuizImg/HindiAudio/थ17.mp3'
import Audio8 from '../../../../assets/QuizImg/HindiAudio/द18.mp3'
import Audio9 from '../../../../assets/QuizImg/HindiAudio/ध19.mp3'
import Audio10 from '../../../../assets/QuizImg/HindiAudio/न20.mp3'
...
const HindiEasyQ3 = () => {
    ...
    // Get Quiz Ans Data
    useEffect(() => {
        fetch(`https://langzila.herokuapp.com/hindiEasyQAns3/${email}`, {
            method: 'GET'
        })
            .then(res => res.json())
            .then(data => {
    ...
    const selectAnswer = async (quizAns, quz, Allans, isData) => {
        ...
        if (!isData) {
            ...
            if (QuizData) {
                try {
                    const { data } = await axios.post(`https://langzila.herokuapp.com/hindiEasyQ3`, QuizData, {
                        method: 'POST'
                    });
                    ...
                    showScore ?
                        ...
                        <div className='h-screen  w-full bg-white flex flex-col items-center justify-center'>
                            ...
                            <div className='xl:w-1/2 w-2/3'>
                                <div className=' text-left text-lime-600 text-xl font-bold'>{count + 1} <span className=' text-black'>/</span> {quizs.length}</div>
                                <div className=' w-40 h-40 mx-auto p-5 bg-white border border-gray-300 mt-7 text-gray-700 shadow-md shadow-lime-200 rounded text-8xl text-bold font-mono flex items-center justify-center relative z-20'>
                                    {quizs[count]?.Latter}
                                ...
                                <div className=' grid grid-cols-2 gap-6 mt-10'>

                                    {
                                        quizs[count]?.quiz_answers?.map((quizAns, idx) => <button onClick={() => { selectAnswer(quizAns, { quz: quizs[count]?.Latter }, { AllAns: (quizs[count]?.quiz_answers) }, isData) }} key={idx}
                                            className={` p-4 bg-lime-500 text-white hover:bg-black hover:text-white rounded shadow-md text-lg font-semibold transition duration-200`}>{quizAns?.answere}</button>)
                                    }
                                </div>
                            </div>
                            <div className=' mt-5'>
                                <progress className="progress progress-success  xl:w-[870px] w-[550px] " value={progressValue} max="100"></progress>
                            </div>
                        </div>
            }
        </div >
